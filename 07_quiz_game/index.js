// Array med quizspørgsmål i JSON basicly json objekt notation (notation)
let questions = [
  {
...
function setup(){
  ...
  //vi vælger html elementet med id "question"
  // indsætter først objektes "spørgsmål" i den
...
function showQ(){
    //hvis man kalder på true kalder man funktionen checkAnswer med argumentet true
    select('#question').html(questions[q].spørgsmål)
}
...
function checkAnswer(bool){
  ...
  if (questions.length == q){
    select('#resulat').html(`YAYYY, du fik ${score} point`)
    shiftPage('#page3')
    return 
  }
  showQ()
