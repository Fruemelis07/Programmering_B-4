...
function setup(){
    ...
    recBtn.mousePressed(()=>{
        if(!isRecording){
            ...
            recorder.stop()
            isRecording = false
            recBtn.htmnl("start recording")
            setTimeout(()=>{
                audioFile.play()
                SVGAElement(audioFile,"myvoice.wav")
            }, 500)
    ...
    speakButtonn = select('#speakButton')
    //Når man trykker på knappen, læses indholdet i inpiut feltet op
    speakButtonn.mousePressed(()=>{
