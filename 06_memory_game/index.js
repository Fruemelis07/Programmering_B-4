...
function setup(){
    ...
    select('#startGame').mousePressed(()=>{
        setupGame() //Når man trykker på start game buttom, så starter man spillet
...
function setupGame(){
    ...
    images = shuffle(images) //tager et array og blander elementerne
    console.log(images) //viser hvad vi for ud af det
    images.map ( i => {
        //DOM binding til at spille conatiner
        var container = select ('#gameContainer')
        //opret spilletkortene div, læg dem ind i gamecontainer ogput et billede ind i div'erne
        var card = createElement('div').addClass('card').attribute('img-source', i).parent(container).child(createImg(i)).mousePressed(()=>{
            if (flippedCards.length <2) {
                 card.addClass('show')
                flippedCards.push(card)
                //hvis der er to kort i flippedCards, skal vi tjekke om de matcher
                if (flippedCards.length ==2){
                    ...
                    if(flippedCards[0].attribute('img-source') == flippedCards[1].attribute('img-source')){
                        ...
                        flippedCards = []
                        }, 1500) //vent 1,5 sekunder før kortene vender igen
